...
$(function () {
    ...
    $("#btn_create").on("click", function () {
        ...
        //$("#VoteCount").val("3");
        $("#VoteTitle").val("");
        $("#VoteTitle").prop("readonly", "").prop('disabled', '');
        $("#Sponsor").prop("readonly", "").prop('disabled', '');

        for (var i = 0; i < 3; i++) {
            $("#vote" + (i + 1)).val("");
            $("#vote" + (i + 1)).prop("readonly", "").prop('disabled', '');
            $("#vote" + (i + 1)).css("background-color", "");
        }
...
function submitParty() {
    ...
    var beginTime = new Date($("#addBeginTime").val());
    var endTime = new Date($("#addEndTime").val());
    var result = endTime - beginTime;
    if (result < 3600000) {
        $(".time-error").show();
        return false;
    } else {
    ...
    if (validator.numberOfInvalids() <= 0) {
        ...
        //var beginTime = new Date(Date.parse($("#addBeginTime").val()) + localOffset);
        //$("#addBeginTime").val(beginTime.Format("yyyy-MM-dd HH:mm"));
        //var endTime = new Date(Date.parse($("#addEndTime").val()) + localOffset);
...
function editParty(row) {
    ...
    var endDate = Date.parse(row.EndTime);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#ParticipantCount").prop('disabled', 'disabled');
        }
    });
    ...
    if (JSON.parse(row.VoteChoicesJson).length>0) {
        ...
        $("#VoteTitle").prop("readonly", "readonly").prop('disabled', 'disabled');
        ...
        for (var i = 0; i < voteChoices.length; i++) {
            ...
            $("#vote" + (i + 1)).prop("readonly", "readonly").prop('disabled', 'disabled');
            $("#vote" + (i + 1)).css("background-color", "#eee");
        }
    } else {
        $(".view-link").hide();
    }

    $("#Sponsor").prop("readonly", "readonly");
    $("#Sponsor").css("background-color", "#eee").prop('disabled','disabled');
...
function showComments(row) {
    ...
    $("#" + gridPartyCommentId).dataMatrix({
        ...
        properties: [
            { type: "datetime", name: "CreatedTime", displayName: "创建时间", isColumnHidden: true, isSortable: true, isSearchable: false },
