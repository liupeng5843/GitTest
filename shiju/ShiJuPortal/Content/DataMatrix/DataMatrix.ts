...
// For simple datetimepicker
interface Date {
    addMinutes(delta: number): Date;
...
interface JQuery {
    ...
    datetimepicker(methodName: 'destroy'): void;
    datetimepicker(methodName: 'dialog', date?: Date, onSelect?: () => void, pos?: any): void;
    datetimepicker(methodName: 'dialog', date?: string, onSelect?: () => void, pos?: any): void;
    datetimepicker(methodName: 'getDate'): Date;
    datetimepicker(methodName: 'hide'): void;
    datetimepicker(methodName: 'isDisabled'): boolean;
    datetimepicker(methodName: 'refresh'): void;
    datetimepicker(methodName: 'setDate', date: Date): void;
    datetimepicker(methodName: 'setDate', date: string): void;
    datetimepicker(methodName: 'show'): void;
    datetimepicker(methodName: 'widget'): JQuery;
    datetimepicker(methodName: string): JQuery;
    datetimepicker(options: DateTimePickerOptions): JQuery;
    datetimepicker(optionLiteral: string, optionName: string): any;
    datetimepicker(optionLiteral: string, options: DateTimePickerOptions): any;
    datetimepicker(optionLiteral: string, optionName: string, optionValue: any): JQuery;
}


class DataMatrixProperty {
    ...
    public isColumnHidden: boolean; // The column is hidden temporarily. It can be shown after being choosen.
    public isColumnForbidden: boolean; // The column cannot be choosen
}

class DataMatrixFooterBarButton {
    public displayName: string;
    public description: string;
    public buttonClass: string;
    public onClick: (button: DataMatrixFooterBarButton, selectedRowIds: any[], selectedRows: any[]) => void;
}

enum DataMatrixRowActionButtonType {
    Button = 0,
    Link = 1,
}

class DataMatrixRowActionButton {
    public buttonType: DataMatrixRowActionButtonType;
    public displayName: string;
    public buttonClass: string;
    public icon: string;
    public onClick: (button: DataMatrixRowActionButton, rowId: any) => void;
}

class DataMatrixOptions {
    public implementationType: string;  // jqGrid(default) or bootGrid
    ...
    public isColumnChooserEnabled: boolean;
    public rowCountPerPage: number;
    public rowCountsPerPage: Array<number>;
    public oDataUrl: string;
    public extraRequestData: any;
    public properties: DataMatrixProperty[];
    public defaultSortPropertyName: string;
    public defaultSortAscending: boolean;
    public actionButtons: DataMatrixRowActionButton[];
    public footerBarButtons: DataMatrixFooterBarButton[];
    public isHeaderBarHidden: boolean;   // default: true
    public isFooterBarHidden: boolean;   // default: true
}

interface DataMatrixPropertyTypePlugin {
    name: string;
    RenderCellHtml(id: any, dataRow: any, dataCell: any, propertyTypeOptions: { [key: string]: any; }): string;
    RenderInputHtml(id: any, dataRow: any, dataCell: any, propertyTypeOptions: { [key: string]: any; }): string;
}



interface JQuery {
    jqGrid(options: any): JQuery;
    jqGrid(optionLiteral: string, optionName: string): any;

    bootgrid(options: any): JQuery;

    dataMatrix(): JQuery;
    dataMatrix(options: DataMatrixOptions): JQuery;
    dataMatrix(methodName: 'option'): DataMatrixOptions;
    dataMatrix(methodName: string): JQuery;
    dataMatrix(optionLiteral: string, optionName: string): any;
    dataMatrix(optionLiteral: string, options: DataMatrixOptions): any;
    dataMatrix(optionLiteral: string, optionName: string, optionValue: any): JQuery;
}


class DataMatrixStatic {
    public static propertyTypePlugins: { [key: string]: DataMatrixPropertyTypePlugin; };
}
...
$.widget("ui.dataMatrix", {
    ...
    deleteRow: function (rowId: any, needConfirmation: boolean) {
        switch (this.options.implementationType.toLowerCase()) {
            case "bootgrid":
    ...
    reloadGrid: function () {
        switch (this.options.implementationType.toLowerCase()) {
            ...
            default:    // default: jqgrid
            //return DataMatrixJqGridImplementation.reload(this._jqgrid, this.options);
        }
    }
});
...
module DataMatrixJqGridImplementation {

    var dateFormat = function () {
        var token = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g,
            timezone = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g,
            timezoneClip = /[^-+\dA-Z]/g,
            pad = function (val, len = 2) {
                val = String(val);
        ...
        return function (date, mask, utc = false) {
            var dF = dateFormat;
    ...
    function encodingXml(str: string): string {
        str = str.replace(/\&/g, "&amp;");
        str = str.replace(/\"/g, "&quot;");
        str = str.replace(/\</g, "&lt;");
        str = str.replace(/\>/g, "&gt;");
    ...
    function endsWithString(str: string, end: string): boolean {
        return (str.substring(str.length - end.length) === end);
    }

    function startsWithString(str: string, start: string): boolean {
        return (str.substr(0, start.length) === start);
    }

    function combineUrl(url1: string, url2: string): string {
        var url: string = endsWithString(url1, "/") ? url1 : url1.concat("/");
        return url.concat(startsWithString(url2, "/") ? url2.substring(1) : url2);
    }

    function getKeyPropertyName(properties: DataMatrixProperty[]) {
    ...
    function getHeaders(properties: DataMatrixProperty[], options: DataMatrixOptions) {
    ...
    function getColumDefinitions(properties: DataMatrixProperty[]) {
        return $.map(properties, function (property: DataMatrixProperty, idx) {
            var col = {
            ...
            switch (property.type) {
                ...
                case "multilinestring":
                    col = $.extend(true, {
                        edittype: 'textarea',
                        editoptions: { rows: "5", cols: "dummy" }, // Setting 'cols' 'dummy' value is a workaround to avoid jqGrid setting it to '20'.
                        searchoptions: { sopt: ['eq', 'ne', 'lt', 'le', 'gt', 'ge', 'bw', 'bn', 'ew', 'en', 'cn', 'nc', 'nu', 'nn'] },
                        searchrules: {},
                        cellattr: function (rowId, tv, rawObject, cm, rdata) {
                            return 'style="white-space: normal; vertical-align: middle;"';
                            ...
                            dataInit: function (el) {
                                setTimeout(function () {
                                    ...
                                    jQuery(el).datepicker({
                                        dateFormat: 'yy-mm-dd', minDate: new Date(1900, 0, 1), maxDate: new Date(2099, 12, 31), showOn: 'focus', showButtonPanel: true, changeMonth: true, changeYear: true
                                    });
                                }, 200);
                            }
                        },
                ...
                case "imagehub":
                    col = $.extend(true, {
                        ...
                        editoptions: {
                            custom_element: function (value, options) {
                                ...
                                var html = "<object id=\"" + silverlightId + "\" data=\"data:application/x-silverlight-2,\" type=\"application/x-silverlight-2\" width=\"100%\" height=\"100\">"
                                    + "<param name=\"source\" value=\"" + imageUploadKitUrl + "\" />"
                                    + "<param name=\"onError\" value=\"onSilverlightError\" />"
                                    + "<param name=\"background\" value=\"white\" />"
                                    + "<param name=\"minRuntimeVersion\" value=\"5.0.61118.0\" />"
                                    + "<param name=\"windowless\" value =\"true\" / >"
                                    + "<param name=\"initparams\" value=\"ImageServiceUrl=" + property.typeOptions["baseUrl"] + ", AppIdOrName=" + property.typeOptions["appIdOrName"] + ",HtmlValueElementId=" + valueElementId + ",IsMultiple=" + (property.typeOptions["isMultiple"] ? "True" : "False") + "\" />"
                                    + "<param name=\"autoUpgrade\" value=\"true\" />"
                                    + "<a href=\"http://go.microsoft.com/fwlink/?LinkID=149156&v=5.0.61118.0\" style=\"text-decoration:none\">"
                                    + "<img src=\"http://go.microsoft.com/fwlink/?LinkId=161376\" alt=\"Get Microsoft Silverlight\" style=\"border-style:none\" />"
                                    + "</a></object>"
                                    + "<iframe id=\"_sl_historyFrame\" style=\"visibility:hidden;height:0px;width:0px;border:0px\"></iframe>"
                                    + "<input id=\"" + valueElementId + "\" type=\"hidden\" value=\"" + encodingXml(value) + "\" width=\"600\" />";
                                $(elem).html(html);
                    ...
                    break;
                default:
                    var plugin: DataMatrixPropertyTypePlugin = DataMatrixStatic.propertyTypePlugins[property.type];
                    if (plugin !== undefined) {
                        col = $.extend(true, {
                            cellattr: function (rowId, tv, rawObject, cm, rdata) {
                                return 'style="vertical-align: middle;"';
    ...
    function ODataListExpression(comparer, field, dataItems, filter) {
        var quoteData = $.grep(filter, function (element: any, idx) {
    ...
    /// cols is an array.
    function getQuoteFlags(properties: Array<DataMatrixProperty>) {
        return $.map(properties, function (property: DataMatrixProperty, idx) {
            //sortCols
            var quoteFlags;
            ...
            switch (property.type) {
                ...
                case "enumstring":
                    ...
                    break;
                case "enumguid":
                    quoteFlags = { col: property.name, isList: false, convert: function (v) { return "guid'" + v + "'"; } };
                    break;
                case "date":
                    quoteFlags = { col: property.name, isList: false, convert: function (v) { return "DateTime'" + v + "T00:00:00'"; } };
                    break;
                case "datetime":
                    quoteFlags = { col: property.name, isList: false, convert: function (v: string) { var t = dateFormat(new Date(v).addMinutes(-(new Date().getTimezoneOffset())), "yyyy-mm-dd HH:MM:ss"); return "DateTime'" + t.replace(" ", "T") + "'"; } };
                    break;
                case "imageurl":
                    quoteFlags = { col: property.name, quoteValue: true, isList: false };
                    break;
                ...
                default:
                    var plugin: DataMatrixPropertyTypePlugin = DataMatrixStatic.propertyTypePlugins[property.type];
                    if (plugin !== undefined) {
                        // Currently, plugin does not support search.
                    }
                    else {
                        debugger;
                    }
    ...
    function buildColumnSort(gridColCommand, filters) {
        ...
        var quoteData = $.grep(filters, function (element: any, idx) {
    ...
    function findQuoteFilter(field, filters) {
        var quoteFilter = $.grep(filters, function (element: any, idx) {
            return element.col === field;
        });
    ...
    class JqGridGlobal {
        public static jqGridSettings: any[] = [];
    }

    function bindConfiguration(key, options: DataMatrixOptions) {
        ...
        if (p === null || p === undefined) {
            ...
            p = $.extend(true, {
                ...
                gridHeight: (options.height === undefined) ? 500 : undefined, // Obsolete property. 'gridHeight' should be removed in future.
    ...
    export function generateGrid(options: DataMatrixOptions) {
    ...
    function setupActionColumn(grid, options: DataMatrixOptions) {
        if (!(options.actionButtons) || (options.actionButtons.length <= 0)) {
            return;
        }
        ...
        for (var i = 0; i < ids.length; i++) {
            ...
            for (var k = 0; k < actionButtonLength; k++) {
                ...
                if ((!actionButton.buttonType) || (actionButton.buttonType == DataMatrixRowActionButtonType.Button)) {
                    html += "<button class=\"" + actionButton.buttonClass
                        + "\" onclick=\"DataMatrixJqGridImplementation.handleActionButtonClicked('" + options.elementId + "'," + k + ",'" + rowId + "');\" >"
                    if (actionButton.icon) {
                        html += "<span class=\"" + actionButton.icon + "\"></span>"
                    }
                    html += encodingXml(actionButton.displayName) + "</button>";
                }
                else if (actionButton.buttonType == DataMatrixRowActionButtonType.Link) {
                    html += "<button class=\"btn btn-sm btn-link " + actionButton.buttonClass
                        + "\" onclick=\"DataMatrixJqGridImplementation.handleActionButtonClicked('" + options.elementId + "'," + k + ",'" + rowId + "');\" >"
                    html += encodingXml(actionButton.displayName) + "</button>";
                }
                else {
    ...
    export function handleActionButtonClicked(dataMatrixElementId: string, actionButtonIndex: number, rowId: string) {
        var options: DataMatrixOptions = $("#" + dataMatrixElementId).dataMatrix("option");
    ...
    function appendExtraRequestData(baseUrl: string, options: DataMatrixOptions) {
    ...
    function serverErrorTextFormat(data: JQueryXHR) {
    ...
    function buildNavigation(grid, options: DataMatrixOptions) {
        ...
        grid.navGrid(
            "#" + p.pager.id,
            { search: p.originalOptions.isSearchButtonVisible, edit: p.originalOptions.isEditButtonVisible, add: p.originalOptions.isCreateButtonVisible, del: p.originalOptions.isDeleteButtonVisible },
            editOptions, // default settings for edit
            addOptions, // default settings for add
            delOptions, // delete
            buildSearchOptions(grid, options)
        );
        ...
        for (var i = 0; i < footerBarButtonLength; i++) {
            grid.jqGrid(
                'navButtonAdd',
                "#" + p.pager.id,
                createFooterButtonObject(grid, options, options.footerBarButtons[i]));
        }
    };

    function buildSearchOptions(grid, options: DataMatrixOptions) {
        return { closeOnEscape: true, multipleSearch: true, closeAfterSearch: true, multipleGroup: true };
    }

    export function getSelectedRowIds(grid, options: DataMatrixOptions) {
        var selectedRowIds: any[] = grid.jqGrid('getGridParam', 'selarrrow');
    ...
    export function getRow(grid, rowId: any) {
        var rowData = grid.jqGrid('getRowData', rowId);
        return rowData;
    }

    export function deleteRow(grid, options: DataMatrixOptions, rowId: any, needConfirmation: boolean) {
        var rowIds = [];
        rowIds.push(rowId);
        deleteRows(grid, options, rowIds, needConfirmation);
    }

    export function deleteRows(grid, options: DataMatrixOptions, rowIds: any[], needConfirmation: boolean) {
        var key = grid.context.id.split("-")[0];
    ...
    export function editRow(grid, options: DataMatrixOptions, rowId: any) {
        var key = grid.context.id.split("-")[0];
        var jqGridOptions = JqGridGlobal.jqGridSettings[options.idPrefix];
    ...
    export function createRow(grid, options: DataMatrixOptions) {
        //var key = grid.context.id.split("-")[0];
        //var jqGridOptions = JqGridGlobal.jqGridSettings[options.idPrefix];
        //var p = bindConfiguration(key, jqGridOptions);
        var addOptions: any = {
    ...
    export function showSearchDialog(grid, options: DataMatrixOptions) {
        grid.jqGrid('searchGrid', buildSearchOptions(grid, options));

    }


    function createFooterButtonObject(grid, options: DataMatrixOptions, button: DataMatrixFooterBarButton) {
        return {
            caption: button.displayName,
            title: button.description,
            'class': button.buttonClass,
            onClickButton: function () {
                var selectedRowIds: any[] = getSelectedRowIds(grid, options);
                var selectedRows = [];
    ...
    function parseFilterGroup(filterGroup, filters) {
        ...
        if (filterGroup.rules.length) {
            ...
            // odata expression.
            var listFields = $.grep(filterGroup.rules, function (rule: any, idx) {
            ...
            $.each(distinctFieldNames, function (idx, fieldName) {
                ...
                var fieldValues = $.grep(listFields, function (fieldValue: any, idx) {
            ...
            var elementFields = $.grep(filterGroup.rules, function (rule: any, idx) {
                var foundFilter = findQuoteFilter(rule.field, filters);
                if (foundFilter.isList !== undefined) {
                    return !foundFilter.isList;
                }
...
module DataMatrixBootgridImplementation {

    var bootGridEventNameSpace = ".rs.jquery.bootgrid";

    class BootgridOptions {
        ...
        public rowCount: any;   // Array or Integer
        public selection: boolean;
        public multiSelect: boolean;
        public sorting: boolean;
        public multiSort: boolean;
        public rowSelect: boolean;
        public keepSelection: boolean;
        public ajax: boolean;
        public post: any;   // PlainObject or Function
        public extraQueryString: any;
        public url: any;    // String or Function
        public caseSensitive: boolean;
        public requestHandler: any; // Function
        public responseHandler: any;    // Function
    ...
    class BootgridProperty {
        ...
        public buttons: DataMatrixRowActionButton[];
    }

    class BootgridPager {
        public static Current: number = 1;
        public static RowCount: number = 20;
        public static Rows: any = [];
        public static Total: number = 0;
    }

    function generateHeader(properties: BootgridProperty[]) {
        var headRow = $("<tr>");
        properties.forEach(function (p: BootgridProperty, i: number, properties): void {
    ...
    export function generateGrid(options: DataMatrixOptions) {
        var $container = $("#" + options.elementId),
            gridId = options.idPrefix + "-grid",
            $table = $("<table id=\"" + gridId + "\" class=\"table-hover\" style=\"table-layout: auto;\">"),
            bootGridProperties = convertToBootgridProperties(options),
            bootGridOptions = convertToBootgridOptions(options, bootGridProperties),
            $thead = generateHeader(bootGridProperties);
        if (typeof (options.height) !== "undefined") {
            $container.css({ "height": options.height + "px", "overflow-x": "hidden" });
        }
        $container.append($table.append($thead));

        var grid = $table.bootgrid(bootGridOptions)
            .on("loaded" + bootGridEventNameSpace, function () {
    ...
    function convertToBootgridOptions(options: DataMatrixOptions, bootgridProperties: BootgridProperty[]): BootgridOptions {
    ...
    function convertToBootgridProperties(options: DataMatrixOptions): BootgridProperty[] {
        var properties: BootgridProperty[] = [];

        var dataMatrixProperties = options.properties;

        dataMatrixProperties.forEach(function (p: DataMatrixProperty, i, dataMatrixProperties): void {
            ...
            if ((options.defaultSortPropertyName || "").toLowerCase() === property.id.toLowerCase()
                || (options.defaultSortPropertyName || "").toLowerCase() === property.displayName.toLowerCase()) {
                property.sortable = true;
    ...
    function generateConverters(bootgridProperties: BootgridProperty[]) {
        var converters = {};

        bootgridProperties.forEach(function (p: BootgridProperty, i, bootgridProperties) {
            switch (p.type.toLowerCase()) {
                ...
                case "date":
                    converters[p.converter] = {
                        ...
                        to: function (value) { return dateFormat(value, "yyyy-mm-dd"); }
                    };
                    break;
                case "datetime":
                    converters[p.converter] = {
    ...
    function generateFomatters(bootgridProperties: BootgridProperty[]) {
        var formatters = {};
        bootgridProperties.forEach(function (p: BootgridProperty, i, bootgridProperties) {
            switch (p.type.toLowerCase()) {
                case "action":
    ...
    export function handleActionButtonClicked(dataMatrixElementId: string, actionButtonIndex: number, rowId: any) {
        var options: DataMatrixOptions = $("#" + dataMatrixElementId).dataMatrix("option");
        var actionButton = options.actionButtons[actionButtonIndex];
        actionButton.onClick(actionButton, rowId);
    }
    ...
    function actionFormatter(column, row) {
        var rowId = (column.identifierColumnId === null) ? row._rowIndex : row[column.identifierColumnId],
            actionButtons = column.buttons,
            elementId = column._elementId,
            html = "";
        ...
        for (var i = 0; i < actionButtons.length; i++) {
            ...
            if ((!actionButton.buttonType) || (actionButton.buttonType == DataMatrixRowActionButtonType.Button)) {
                html += "<button class=\"" + actionButton.buttonClass
                    + "\" onclick=\"DataMatrixBootgridImplementation.handleActionButtonClicked('" + elementId + "'," + i + ",'" + rowId + "');\" >"
                if (actionButton.icon) {
                    html += "<span class=\"" + actionButton.icon + "\"></span>"
                }
    ...
    function generateImageUrl(baseUrl: string, appIdOrName: string, imageId: string, scaleType: number, width: number, height: number, extension: string) {
    ...
    function enumFormatter(column, row) {
        var cellvalue = row[column.id],
            typeOptions = column.typeOptions,
            enumValues = typeOptions["enumValues"],
            enumHtmlValues = typeOptions["enumHtmlValues"];
