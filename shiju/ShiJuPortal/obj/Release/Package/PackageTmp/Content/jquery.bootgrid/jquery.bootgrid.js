...
;(function ($, window, undefined)
{
    ...
    function loadColumns()
    {
        ...
        firstHeadRow.children().each(function ()
        {
            var $this = $(this),
                data = $this.data(),
                column = {
                    _elementId: that.options._elementId,
                    id: data.columnId,
                    identifier: that.identifier == null && data.identifier || false,
                    type: data.type || "string",    //*** default: 'string'
                    typeOptions: data.typeOptions,  //***
                    columnWidth: data.columnWidth,
                    buttons: data.buttons || [],  //*** 'action' type column has some buttons
            ...
            if (column.identifier) {
                ...
                //*** in order to get rowId for each columns
                $.each(that.columns, function (i, c) {
                    c.identifierColumnId = that.identifier;
                });
            } else {
                column.identifierColumnId = that.identifier;    //*** in order to get rowId for each columns
            }
    ...
    //*** to support odata ***//
    function appendODataQuery(url, request) {
        var that = this;
        ...
        if (request.rowCount > 0) {
            url += ("$top=" + request.rowCount) + "&";
            if (request.current > 1) {
                url += ("$skip=" + ((request.current - 1) * request.rowCount)) + "&";
            }
        }
        ...
        if (request.searchPhrase.length > 0) {
            ...
            if (searchableColumns.length > 0) {
                for (var i = 0; i < searchableColumns.length; i++) {
                    var column = searchableColumns[i];
                    var encodeKeyword = encodeURIComponent(request.searchPhrase);
                    switch (column.type.toLowerCase()) {
                        case "string":
                            url += ("substringof('" + encodeKeyword + "', " + column.id + ") eq true");

                            if (i < searchableColumns.length - 1) {
                                url += " or ";  // 'or' case sensitive
                            }
                            break;
                        ...
                        default:
                            //url += "1 eq 1";
                            if (url.substr(url.length - 4, 4) === " or ") {
        ...
        // append $orderby...
        if ($.isArray(request.sort) && request.sort.length > 0) {
            url += "$orderby=";
            for (var i = 0; i < request.sort.length; i++) {
                for (var pair in request.sort[i]) {
                    url += ("" + pair + " " + request.sort[i][pair]);
                }
                if (i < request.sort.length - 1) {
                    url += ", ";
                }
            }
            url += "&";
        } else {
            if ($.isEmptyObject(request.sort) === false) {
                url += "$orderby=";
                for (var pair in request.sort) {
                    url += ("" + pair + " " + request.sort[pair]) + ",";
                }
                if (url.substr(url.length - 1, 1) === ",") {
                    url = url.substr(0, url.length - 1);
                }
        ...
        return url + "$inlinecount=allpages";
    }

    //***
    function appendOtherQueryString(url) {
        var that = this;

        var extraQueryString = this.options.extraQueryString;
        if (typeof (extraQueryString) !== "undefined" && extraQueryString !== {}) {
            ...
            for (var q in extraQueryString) {
                url += q + "=" + extraQueryString[q] + "&";
            }
        }
    ...
    */
    ...
    function loadData()
    {
        ...
        if (this.options.ajax)
        {
            ...
            /*** due to post request in odata is create a new record, use instead ajax ***/
            ...
            url = appendOtherQueryString.call(that, url);
            url = appendODataQuery.call(that, url, request);
    ...
    function renderActions()
    {
        if (this.options.navigation !== 0)
        {
            ...
            if (this.options.navigation === 1 || this.options.navigation === 3) {
                headerActions = this.header.find(selector);
            }
            if (this.options.navigation === 2 || this.options.navigation === 3) {
    ...
    function renderRows(rows)
    {
        if (rows.length > 0)
        {
            ...
            $.each(rows, function (index, row)
            {
                ...
                rowCss = "";
                //***
                row = $.extend(true, row, { _rowIndex: index });
                ...
                {
                    row[column.id] = column.converter.to(column.converter.from(row[column.id]));
                    if (column.visible)
                    {
                        var value = ($.isFunction(column.formatter)) ? 
                                column.formatter.call(that, column, row) : 
                                    column.converter.to(column.converter.from(row[column.id])), //***
                            cssClass = (column.cssClass.length > 0) ? " " + column.cssClass : "";
                        if (typeof (column.columnWidth) !== "undefined") {
                            tpl.cell = $(tpl.cell).css("max-width", column.columnWidth + "px").get(0).outerHTML;
                        }
    ...
    function renderTableHeader()
    {
        ...
        $.each(this.columns, function (index, column)
        {
            if (column.visible)
            {
                ...
                if (typeof (column.columnWidth) !== "undefined") {
                    tpl.headerCell = $(tpl.headerCell).css("max-width", column.columnWidth + "px").get(0).outerHTML;
                }
    ...
    Grid.defaults = {
        ...
        templates: {
            ...
            body: "<tbody></tbody>",
            cell: "<td class=\"{{ctx.css}}\" style=\"vertical-align: middle;\">{{ctx.content}}</td>",
            footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\" style=\"padding:0;\"><div class=\"row\"><div class=\"col-sm-6 infoBar\" style=\"text-align: left;\"><p class=\"{{css.infos}}\"></p></div><div class=\"col-sm-6\" style=\"text-align: right;\"><p class=\"{{css.pagination}}\"></p></div></div></div>",
    ...
    ***/
    ...
    Grid.prototype.deleteRowData = function (rowId) {
        var that = this,
            url = that.options.url + "(" + rowId + ")",
            url = appendOtherQueryString.call(that, url);
